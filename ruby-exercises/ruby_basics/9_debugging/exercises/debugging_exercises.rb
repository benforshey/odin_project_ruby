...
def decrement_smallest_value(nested_array, dec_by = 1)
  nested_array.flatten.min - dec_by
end

# use the stack trace to debug the following method
# Don't look at this method prior to running the test
# Run rspec, let the test fail, and go through the stack trace again
def increment_greatest_value(nested_array, inc_by = 1)
  nested_array.flatten.max + inc_by
end
